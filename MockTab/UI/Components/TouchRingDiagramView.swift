...
struct TouchRingDiagramView: View, Equatable {
    ...
    // Quarter slots, indexed 0..3 → bottom-left, top-left, top-right, bottom-right
    // (Mode 1 LED maps to the bottom-left quadrant; modes advance
    //  counter-clockwise.  SVG IDs ring-slot-1..4 are TL/TR/BR/BL, so we list
    //  them here as 4, 1, 2, 3 to align with the LED indicator order.)
    ...
    // Third slots, indexed 0..2 → bottom, top-left, top-right
    // (Modes advance counter-clockwise from the bottom wedge.  SVG IDs
    //  ring-slot-1..3 are TL/TR/B, so we list them as 3, 1, 2 to align with
    //  the LED indicator order.)

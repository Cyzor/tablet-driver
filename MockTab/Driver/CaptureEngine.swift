...
final class CaptureEngine: ObservableObject {
  ...
  private var stepTimeoutTimer: Timer?
    ...
    private var discoveryDuration: TimeInterval = 60
  ...
  func startCalibration(deviceInfo: CaptureDeviceInfo, steps: [CalibrationStep]) {
    reset()
  ...
  func recordSample(reportID: UInt8, report: UnsafePointer<UInt8>, length: Int) {
    ...
    // Auto-advance after a short delay so the UI can show the result.
    stepTimeoutTimer?.invalidate()
    stepTimeoutTimer = Timer.scheduledTimer(withTimeInterval: 0.8, repeats: false) { [weak self] _ in
      Task { @MainActor in
  ...
  func finish() -> CalibrationResult? {
    ...
    func startDiscovery(deviceInfo: CaptureDeviceInfo, duration: TimeInterval = 60) {
        ...
        discoveryTimer = Timer.scheduledTimer(withTimeInterval: duration, repeats: false) { [weak self] _ in
            Task { @MainActor in
        ...
        for (reportID, samples) in discoverySamples {
            ...
            if !samples.isEmpty {
                ...
                for byteIdx in 0..<firstSample.count {
                    let valuesAtIdx = Set(samples.compactMap { byteIdx < $0.count ? $0[byteIdx] : nil })
            ...
            reportSummaries[idHex] = DiscoveryReportSummary(
                ...
                constantBytes: (0..<length).filter { !varyingBytes.contains($0) }
            )
        }
  ...
  private func advanceToStep(_ index: Int) {
    ...
    // Timeout per step — 15 seconds.
    stepTimeoutTimer?.invalidate()
    stepTimeoutTimer = Timer.scheduledTimer(withTimeInterval: 15.0, repeats: false) { [weak self] _ in
      Task { @MainActor in
  ...
  private func buildResult(deviceInfo: CaptureDeviceInfo) -> CalibrationResult {
    ...
    for (idHex, summary) in reportsSeen {
      ...
      let bestSample = summary.samples.max(by: { $0.changedIndices.count < $1.changedIndices.count })
      ...
      for idx in summary.allChangedIndices.sorted() {
        ...
        let changedIn = summary.samples.filter { $0.changedIndices.contains(idx) }.map { $0.step.shortLabel }

...
//
// v3: optional `hid` second argument posts from a
// kCGEventSourceStateHIDSystemState source instead of privateState, so the
// events carry sourceStateID=1 like genuine hardware-originated input.
// Genuine trackpad momentum is generated by the system itself and always
// carries hidSystemState; every synthetic poster tested so far (MockTab,
// Smart Scroll, this probe v2) uses privateState. If macOS 27's column-view
// horizontal recognizer validates momentum-phase events by source state,
// this variant pans horizontally where v2 fails.
//
// Usage:
//   clang -o scroll_momentum_probe scroll_momentum_probe.c \
//       -framework ApplicationServices -framework CoreFoundation
//   ./scroll_momentum_probe horizontal       # or: vertical
//   ./scroll_momentum_probe horizontal hid   # hidSystemState source (v3)
//
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++)
        if (strcmp(argv[i], "hid") == 0) useHIDSource = 1;
    if (useHIDSource) printf("Using kCGEventSourceStateHIDSystemState source.\n");

    CGEventSourceRef source = CGEventSourceCreate(
        useHIDSource ? kCGEventSourceStateHIDSystemState : kCGEventSourceStatePrivate);

...
- `WacomDeviceRegistry`, now public in the package target, with:
  ...
  - `activeWidthMM` / `activeHeightMM` on `WacomDeviceSpec` and derived `lpi: (x:, y:)?`. Hand-measured six `.verified` devices (PTZ-631W, PTH-851, PTH-660, PTH-860 USB + BT, DTK-2400); backfilled 78 entries from libwacom `.tablet` files; filled 22 more using Wacom’s 2540 LPI consumer standard or direct spec sheets. Now 117 of 135 entries include physical dimensions (up from 6). An LPI consistency check rejected libwacom rows that conflicted with `maxX`/`maxY` ratios (including incorrect Movink 13 data), which we replaced with Wacom specs.

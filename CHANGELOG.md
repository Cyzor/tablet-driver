...
- `WacomDeviceRegistry`, now public in the package target, with:
  ...
  - `activeWidthMM` / `activeHeightMM` on `WacomDeviceSpec` and derived `lpi: (x:, y:)?`. Hand-measured six `.verified` devices (PTZ-631W, PTH-851, PTH-660, PTH-860 USB + BT, DTK-2400); backfilled 78 entries from libwacom `.tablet` files; filled 22 more using Wacom’s 2540 LPI consumer standard or direct spec sheets; filled 8 more (Graphire `0x0010`/`0x0011`/`0x0013`/`0x0015`, Volito `0x0060`/`0x0062`, PenStation2 `0x0061`, DTU-1031 `0x00FB`) from `input-wacom` 4.18 features tables using the kernel resolution constants. Now 125 of 135 entries include physical dimensions (up from 6); the remaining 10 are early Graphire/Volito/Bamboo variants whose kernel and OTD records disagree on the PID, plus three wireless receivers (no active area). An LPI consistency check rejected libwacom rows that conflicted with `maxX`/`maxY` ratios (including incorrect Movink 13 data), which we replaced with Wacom specs.
